...
A lightweight Streamlit dashboard is implemented in `app.py` for quickly reviewing portfolio-ready demand intelligence outputs. It loads the scored signal table and video-level score table generated by the scoring pipeline.
...
If dashboard input files are missing, the app shows a clear in-page warning instead of crashing. The dashboard is intended as a recruiter-friendly view of the evaluated analytical prototype, not as a deployed production BI system.
